        ...
        run_pair_array.write_text(
            "\n".join(
                [
                    ...
                    'GLOBAL_IDX=$((PAIR_OFFSET + SLURM_ARRAY_TASK_ID))',
                    'ROW_NUM=$((GLOBAL_IDX + 2))',
                    'ROW_TSV="$("$PYTHON_BIN" - "$PAIR_CSV" "$ROW_NUM" <<\'"'"'PY\'"'"'',
                    'import csv, sys',
                    'pair_csv = sys.argv[1]',
                    'row_num = int(sys.argv[2])',
                    'with open(pair_csv, encoding="utf-8", newline="") as f:',
                    ...
                    ')"',
                    'if [[ -z "$ROW_TSV" ]]; then',
                    '  echo "No row for GLOBAL_IDX=$GLOBAL_IDX (offset=$PAIR_OFFSET, task=$SLURM_ARRAY_TASK_ID)" >&2',
                    '  exit 2',
                    'fi',
                    "IFS=$'\\t' read -r case_id input_type input_path energy_mev spot_idx spot_x_mm spot_y_mm pre_mhd low_out high_out <<< \"$ROW_TSV\"",
                    'if [[ ! -f "$pre_mhd" ]]; then',
